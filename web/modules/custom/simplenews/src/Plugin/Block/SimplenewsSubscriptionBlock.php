...
class SimplenewsSubscriptionBlock extends BlockBase implements ContainerFactoryPluginInterface
{
  ...
  {
    return new static($configuration, $plugin_id, $plugin_definition, $container->get('entity_type.manager'), $container->get('form_builder'));
  }
  ...
  public function defaultConfiguration ()
  {
    ...
    return ['newsletters' => [], 'message' => t('Stay informed - subscribe to our newsletter.'), 'unique_id' => '',];
  }
  ...
  public function blockForm ($form, FormStateInterface $form_state)
  {
    ...
    $form['newsletters'] = ['#type' => 'checkboxes', '#title' => t('Newsletters'), '#options' => $options, '#required' => TRUE, '#default_value' => $this->configuration['newsletters'],];
    $form['message'] = ['#type' => 'textfield', '#title' => t('Block message'), '#size' => 60, '#maxlength' => 255, '#default_value' => $this->configuration['message'],];
    $form['unique_id'] = ['#type' => 'textfield', '#title' => t('Unique ID'), '#size' => 60, '#maxlength' => 255, '#description' => t('Each subscription block must have a unique form ID. If no value is provided, a random ID will be generated. Use this to have a predictable, short ID, e.g. to configure this form use a CAPTCHA.'), '#default_value' => $this->configuration['unique_id'],];
  ...
  public function build ()
  {
    ...
    if ($user = \Drupal::currentUser()) {
      if ($subscriber = simplenews_subscriber_load_by_uid($user->id())) {
        $form_object->setEntity(Subscriber::create(['email' => $user->getEmail()]));
      } else {
        $form_object->setEntity(Subscriber::create(['email' => $user->getEmail()]));
      }
    } else {
