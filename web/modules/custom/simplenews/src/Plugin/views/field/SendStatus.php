...
class SendStatus extends FieldPluginBase
{
  ...
  public function render (ResultRow $values)
  {
    ...
    if ($node->hasField('simplenews_issue')) {
      ...
      if (!empty($message['uri'])) {
        $output['image'] = ['#theme' => 'image', '#uri' => $message['uri'], '#alt' => $message['description'], '#title' => $message['description'], '#getsize' => TRUE,];
      }
      $output['text'] = ['#type' => 'inline_template', '#template' => '<span title="{{ description }}">{{ count }}</span>', '#context' => $message,];
      return $output;
    }
  }
  ...
  protected function getMessage ($node)
  {
    ...
    $message['uri'] = NULL;
    $images = [SIMPLENEWS_STATUS_SEND_PENDING => 'images/sn-cron.png', SIMPLENEWS_STATUS_SEND_READY => 'images/sn-sent.png',];
    if ($status == SIMPLENEWS_STATUS_SEND_READY) {
      ...
      $message['description'] = t('Newsletter issue is pending, @sent_count mails sent out of @count.', ['@sent_count' => $sent_count, '@count' => $subscriber_count]);
      $message['uri'] = drupal_get_path('module', 'simplenews') . '/' . $images[$status];
    } elseif (($status == SIMPLENEWS_STATUS_SEND_NOT)) {
      $message['description'] = t('Newsletter issue will be sent to @count subscribers.', ['@count' => $subscriber_count]);
    }
    if (!$published) {
      $message['description'] = t('Newsletter issue will be sent to @count subscribers on publish.', ['@count' => $subscriber_count]);
    }
    return $message;
  }
}
