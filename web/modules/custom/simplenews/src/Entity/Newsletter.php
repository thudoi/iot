...
class Newsletter extends ConfigEntityBase implements NewsletterInterface
{
  ...
  public static function preCreate (EntityStorageInterface $storage, array &$values)
  {
    ...
    $values += ['format' => $config->get('newsletter.format'), 'priority' => $config->get('newsletter.priority'), 'receipt' => $config->get('newsletter.receipt'), 'from_name' => $config->get('newsletter.from_name'), 'from_address' => $config->get('newsletter.from_address'),];
  ...
  public static function postDelete (EntityStorageInterface $storage, array $entities)
  {
    ...
    $subscription_storage = \Drupal::entityTypeManager()->getStorage('simplenews_subscriber');

    foreach ($entities as $newsletter) {
      $subscription_storage->deleteSubscriptions(['subscriptions_target_id' => $newsletter->id()]);
      drupal_set_message(t('All subscriptions to newsletter %newsletter have been deleted.', ['%newsletter' => $newsletter->label()]));
    }
