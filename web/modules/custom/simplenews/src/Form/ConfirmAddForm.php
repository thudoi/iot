...
class ConfirmAddForm extends ConfirmFormBase
{
  ...
  public function buildForm (array $form, FormStateInterface $form_state, $mail = '', NewsletterInterface $newsletter = NULL)
  {
    ...
    $form['question'] = ['#markup' => '<p>' . t('Are you sure you want to add %user to the %newsletter mailing list?', ['%user' => simplenews_mask_mail($mail), '%newsletter' => $newsletter->name]) . "<p>\n",];
  ...
  public function submitForm (array &$form, FormStateInterface $form_state)
  {
    ...
    if (!$path = $config->get('subscription.confirm_subscribe_page')) {
      ...
      drupal_set_message(t('%user was added to the %newsletter mailing list.', ['%user' => $form_state->getValue('mail'), '%newsletter' => $form_state->getValue('newsletter')->name]));
    }
