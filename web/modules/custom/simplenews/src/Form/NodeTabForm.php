...
class NodeTabForm extends FormBase {
    ...
    if ($status == SIMPLENEWS_STATUS_SEND_NOT) {
      ...
      if ($status == SIMPLENEWS_STATUS_SEND_READY) {
        ...
        if ($status == SIMPLENEWS_STATUS_SEND_PUBLISH) {
          ...
          $form['status']['#title'] = t('This newsletter issue is pending, @count of @total mails already sent.', [
            '@count' => (int) $node->simplenews_issue->sent_count,
            '@total' => \Drupal::service('simplenews.spool_storage')
              ->countMails([
                'entity_type' => 'node',
                'entity_id' => $node->id(),
              ]),
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($form['recipient_handler_settings'])) {
      ...
      $handler_definitions = \Drupal::service('plugin.manager.simplenews_recipient_handler')
        ->getDefinitions();
  ...
  public function submitTestMail(array &$form, FormStateInterface $form_state) {
    \Drupal::service('simplenews.mailer')
      ->sendTest($form_state->get('node'), $form_state->get('test_addresses'));
  }
  ...
  public function submitSendNow(array &$form, FormStateInterface $form_state) {
    ...
    if (\Drupal::service('simplenews.mailer')->attemptImmediateSend([
        'entity_id' => $node->id(),
        'entity_type' => 'node',
      ])) {
  ...
  public function submitStop(array &$form, FormStateInterface $form_state) {
    ...
    drupal_set_message(t('Sending of %title was stopped. @count pending email(s) were deleted.', [
      '%title' => $node->getTitle(),
      '@count' => $count,
    ]));
