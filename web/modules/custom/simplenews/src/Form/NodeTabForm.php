...
class NodeTabForm extends FormBase
{
  ...
  public function buildForm (array $form, FormStateInterface $form_state, NodeInterface $node = NULL)
  {
    ...
    if ($status == SIMPLENEWS_STATUS_SEND_NOT) {
      ...
      $form['test'] = ['#type' => 'details', '#open' => TRUE, '#title' => t('Test'),];
      $form['test']['test_address'] = ['#type' => 'textfield', '#title' => t('Test email addresses'), '#description' => t('A comma-separated list of email addresses to be used as test addresses.'), '#default_value' => \Drupal::currentUser()->getEmail(), '#size' => 60, '#maxlength' => 128,];

      $form['test']['submit'] = ['#type' => 'submit', '#value' => t('Send test newsletter issue'), '#name' => 'send_test', '#submit' => ['::submitTestMail'], '#validate' => ['::validateTestAddress'],];
      $form['send'] = ['#type' => 'details', '#open' => TRUE, '#title' => t('Send'),];
      ...
      $form['send']['recipient_handler'] = ['#type' => 'select', '#title' => t('Recipients'), '#description' => t('Please select to configure who to send the email to.'), '#options' => $options, '#default_value' => $default_handler, '#access' => count($options) > 1, '#ajax' => ['callback' => '::ajaxUpdateRecipientHandlerSettings', 'wrapper' => 'recipient-handler-settings', 'method' => 'replace', 'effect' => 'fade',],];
      ...
      if (method_exists($class, 'settingsForm')) {
        $element = ['#parents' => ['simplenews', 'recipient_handler_settings'], '#prefix' => '<div id="recipient-handler-settings">', '#suffix' => '</div>',];
      ...
      $form['send']['count'] = ['#type' => 'item', '#markup' => t('Send newsletter issue to @count subscribers.', ['@count' => $subscriber_count]),];
      ...
      $form['send']['method'] = ['#type' => 'item', '#markup' => $send_text,];
      if ($node->isPublished()) {
        $form['send']['send_now'] = ['#type' => 'submit', '#button_type' => 'primary', '#value' => t('Send now'), '#submit' => ['::submitForm', '::submitSendNow'],];
      } else {
        $form['send']['send_on_publish'] = ['#type' => 'submit', '#button_type' => 'primary', '#value' => t('Send on publish'), '#submit' => ['::submitForm', '::submitSendLater'],];
      }
    } else {
      $form['status'] = ['#type' => 'item',];
      if ($status == SIMPLENEWS_STATUS_SEND_READY) {
        ...
        $form['actions']['stop'] = ['#type' => 'submit', '#submit' => ['::submitStop'], '#value' => t('Stop sending'),];
      }
    }
    return $form;
  }
  ...
  public function submitSendNow (array &$form, FormStateInterface $form_state)
  {
    ...
    if (\Drupal::service('simplenews.mailer')->attemptImmediateSend(['entity_id' => $node->id(), 'entity_type' => 'node'])) {
      drupal_set_message(t('Newsletter %title sent.', ['%title' => $node->getTitle()]));
    } else {
      drupal_set_message(t('Newsletter issue %title pending.', ['%title' => $node->getTitle()]));
    }
    $node->save();
  }
  ...
  public function submitSendLater (array &$form, FormStateInterface $form_state)
  {
    ...
    drupal_set_message(t('Newsletter issue %title will be sent when published.', ['%title' => $node->getTitle()]));
    $node->save();
  }
  ...
  public function submitStop (array &$form, FormStateInterface $form_state)
  {
    ...
    $count = \Drupal::service('simplenews.spool_storage')->deleteMails(['nid' => $node->id()]);
    ...
    drupal_set_message(t('Sending of %title was stopped. @count pending email(s) were deleted.', ['%title' => $node->getTitle(), '@count' => $count,]));
  ...
  {
    return empty($form['simplenews']['recipient_handler_settings']) ? ['#markup' => '<div id="recipient-handler-settings"></div>'] : $form['simplenews']['recipient_handler_settings'];
  }
