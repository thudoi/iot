...
class PrepareUninstallForm extends FormBase
{
  ...
  {
    $form['simplenews'] = ['#type' => 'fieldset', '#title' => $this->t('Prepare uninstall'), '#description' => $this->t('When clicked all Simplenews data (content, fields) will be removed.'),];

    $form['simplenews']['prepare_uninstall'] = ['#type' => 'submit', '#value' => $this->t('Delete Simplenews data'),];
  ...
  {
    $batch = ['title' => t('Deleting subscribers'), 'operations' => [[[__CLASS__, 'deleteSubscribers'], [],], [[__CLASS__, 'removeFields'], [],], [[__CLASS__, 'purgeFieldData'], [],],], 'progress_message' => static::t('Deleting Simplenews data... Completed @percentage% (@current of @total).'),];
    batch_set($batch);
  ...
  public static function purgeFieldData ()
  {
    do {
      ...
      $properties = ['deleted' => TRUE, 'include_deleted' => TRUE,];
