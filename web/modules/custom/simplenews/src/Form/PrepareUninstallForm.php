...
class PrepareUninstallForm extends FormBase {
  ...
  public static function deleteSubscribers(&$context) {
    $subscriber_ids = \Drupal::entityQuery('simplenews_subscriber')
      ->range(0, 100)
      ->execute();
  ...
  public static function removeFields() {
    $simplenews_fields_ids = \Drupal::entityQuery('field_config')
      ->condition('field_type', 'simplenews_', 'STARTS_WITH')
      ->execute();
    $simplenews_fields = \Drupal::entityManager()
      ->getStorage('field_config')
      ->loadMultiple($simplenews_fields_ids);
    $field_config_storage = \Drupal::entityManager()
      ->getStorage('field_config');
