...
abstract class SubscriptionsFormBase extends ContentEntityForm
{
  ...
  {
    $this->getSubscriptionWidget($form_state)->setAvailableNewsletterIds(array_keys($this->getNewsletters()));
  ...
  protected function actions (array $form, FormStateInterface $form_state)
  {
    ...
    $actions = [static::SUBMIT_SUBSCRIBE => [// Show 'Subscribe' if not subscribed, or user is unknown.
      '#access' => (!$options && !$subscribed) || !$mail, '#type' => 'submit', '#value' => t('Subscribe'), '#submit' => ['::submitForm', '::save', '::submitSubscribe'],], static::SUBMIT_UNSUBSCRIBE => [// Show 'Unsubscribe' if subscribed, or unknown and can select.
      '#access' => (!$options && $subscribed) || (!$mail && $options), '#type' => 'submit', '#value' => t('Unsubscribe'), '#submit' => ['::submitForm', '::save', '::submitUnsubscribe'],], static::SUBMIT_UPDATE => [// Show 'Update' if user is known and can select newsletters.
      '#access' => $options && $mail, '#type' => 'submit', '#value' => t('Update'), '#submit' => ['::submitForm', '::save', '::submitUpdate'],],];
    return $actions;
  }
  ...
  public function validateForm (array &$form, FormStateInterface $form_state)
  {
    ...
    if (\Drupal::currentUser()->isAnonymous() && $user = user_load_by_mail($mail)) {
      $message = $user->isBlocked() ? $this->t('The email address %mail belongs to a blocked user.', ['%mail' => $mail]) : $this->t('There is an account registered for the e-mail address %mail. Please log in to manage your newsletter subscriptions.', ['%mail' => $mail]);
  ...
  {
    return $this->getSubscriptionWidget($form_state)->extractNewsletterIds($form_state->getValue('subscriptions'), $selected);
  }
}
