...
abstract class SubscriptionsFormBase extends ContentEntityForm {
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $actions = [
      static::SUBMIT_SUBSCRIBE => [// Show 'Subscribe' if not subscribed, or user is unknown.
        '#access' => (!$options && !$subscribed) || !$mail,
        '#type' => 'submit',
        '#value' => t('Subscribe'),
        '#submit' => ['::submitForm', '::save', '::submitSubscribe'],
      ],
      static::SUBMIT_UNSUBSCRIBE => [// Show 'Unsubscribe' if subscribed, or unknown and can select.
        '#access' => (!$options && $subscribed) || (!$mail && $options),
        '#type' => 'submit',
        '#value' => t('Unsubscribe'),
        '#submit' => ['::submitForm', '::save', '::submitUnsubscribe'],
      ],
      static::SUBMIT_UPDATE => [// Show 'Update' if user is known and can select newsletters.
        '#access' => $options && $mail,
        '#type' => 'submit',
        '#value' => t('Update'),
        '#submit' => ['::submitForm', '::save', '::submitUpdate'],
      ],
    ];
    return $actions;
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$update && !$this->getSubscriptionWidget($form_state)
        ->isHidden() && !count($form_state->getValue('subscriptions'))) {
