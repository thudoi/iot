...
class ConfirmRemovalForm extends ConfirmFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $mail = '', NewsletterInterface $newsletter = NULL) {
    ...
    $form['question'] = [
      '#markup' => '<p>' . t('Are you sure you want to remove %user from the %newsletter mailing list?', [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    \Drupal::service('simplenews.subscription_manager')
      ->unsubscribe($form_state->getValue('mail'), $form_state->getValue('newsletter')
        ->id(), FALSE, 'website');
    ...
    if (!$path = $config->get('subscription.confirm_unsubscribe_page')) {
      ...
      drupal_set_message(t('%user was unsubscribed from the %newsletter mailing list.', [
        '%user' => $form_state->getValue('mail'),
        '%newsletter' => $form_state->getValue('newsletter')->name,
      ]));
    }
