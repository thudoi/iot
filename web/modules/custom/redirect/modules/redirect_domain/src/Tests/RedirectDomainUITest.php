...
class RedirectDomainUITest extends WebTestBase
{
   ...
   * @var array
   */
  public static $modules = ['redirect_domain',];
  ...
  {
    $user = $this->drupalCreateUser(['administer site configuration', 'access administration pages', 'administer redirects']);
    ...
    $edit = ['redirects[0][from]' => 'foo.example.org', 'redirects[0][sub_path]' => '//sub-path', 'redirects[0][destination]' => 'www.example.org/foo', 'redirects[1][from]' => 'bar.example.org', 'redirects[1][sub_path]' => '', 'redirects[1][destination]' => 'www.example.org/bar',];
