...
class DomainRedirectRequestSubscriberTest extends UnitTestCase {
  ...
  public function testDomainRedirect($request_url, $response_url) {
    $data = [
      'redirect_domain.domains' => [
        'domain_redirects' => [
          'foo:com' => [
            ...
            ['sub_path' => '/*', 'destination' => 'bar.com/example',],
          ],
          'example:com' => [
            [
              'sub_path' => '/foo/*/bar',
      ...
      'redirect.settings' => ['default_status_code' => 301,],
    ...
    if ($response_url) {
      ...
      $this->assertEquals($config_factory->get('redirect.settings')
        ->get('default_status_code'), $response->getStatusCode());
    }
    else {
