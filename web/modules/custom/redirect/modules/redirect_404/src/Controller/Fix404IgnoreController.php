...
class Fix404IgnoreController extends ControllerBase
{
  ...
  {
    return new static($container->get('config.factory'), $container->get('redirect.not_found_storage'));
  }
  ...
  public function ignorePath (Request $request)
  {
    ...
    if (empty($ignored_paths) || !strpos($path, $ignored_paths)) {
      ...
      drupal_set_message($this->t('Resolved the path %path in the database. Please check the ignored list and save the settings.', ['%path' => $path,]));
    }

    $options = ['query' => ['ignore' => $path, 'destination' => Url::fromRoute('redirect_404.fix_404')->getInternalPath(),],];
