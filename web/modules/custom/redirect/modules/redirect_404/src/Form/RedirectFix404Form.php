...
class RedirectFix404Form extends FormBase
{
  ...
  {
    return new static($container->get('language_manager'), $container->get('redirect.not_found_storage'), $container->get('date.formatter'), $container->get('entity_type.manager'));
  }
  ...
  public function buildForm (array $form, FormStateInterface $form_state)
  {
    ...
    $form['basic'] = ['#type' => 'fieldset', '#title' => $this->t('Filter 404s'), '#attributes' => ['class' => ['container-inline']],];
    $form['basic']['filter'] = ['#type' => 'textfield', '#title' => '', '#default_value' => $search, '#maxlength' => 128, '#size' => 25,];
    $form['basic']['submit'] = ['#type' => 'submit', '#value' => $this->t('Filter'), '#action' => 'filter',];
    if ($search) {
      $form['basic']['reset'] = ['#type' => 'submit', '#value' => $this->t('Reset'), '#action' => 'reset',];
    }

    $languages = $this->languageManager->getLanguages(LanguageInterface::STATE_ALL);
    $multilingual = $this->languageManager->isMultilingual();

    $header = [['data' => $this->t('Path'), 'field' => 'source'], ['data' => $this->t('Count'), 'field' => 'count', 'sort' => 'desc'], ['data' => $this->t('Last accessed'), 'field' => 'timestamp'],];
    if ($multilingual) {
    ...
    foreach ($results as $result) {
      ...
      if ($this->entityTypeManager->getAccessControlHandler('redirect')->createAccess()) {
        $operations['add'] = ['title' => $this->t('Add redirect'), 'url' => Url::fromRoute('redirect.add', [], ['query' => ['source' => $path, 'language' => $result->langcode] + $destination]),];
      }
      $row['operations'] = ['data' => ['#type' => 'operations', '#links' => $operations,],];

      $rows[] = $row;
    }

    $form['redirect_404_table'] = ['#theme' => 'table', '#header' => $header, '#rows' => $rows, '#empty' => $this->t('There are no 404 errors to fix.'),];
