...
class RedirectDeleteMultipleForm extends ConfirmFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $this->redirects = $this->privateTempStoreFactory->get('redirect_multiple_delete_confirm')
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('confirm') && !empty($this->redirects)) {
      ...
      $this->privateTempStoreFactory->get('redirect_multiple_delete_confirm')
        ->delete($this->currentUser->id());
      $count = count($this->redirects);
      $this->logger('redirect')
        ->notice('Deleted @count redirects.', ['@count' => $count]);
