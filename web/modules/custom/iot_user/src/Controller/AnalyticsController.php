...
class AnalyticsController extends ControllerBase
{
  ...
  public function dashboard ()
  {
    ...
    return ['#theme' => 'iot_user_analytics', '#user' => $user, '#bandscorechart' => $this->getChartBandScore($user), '#accuracy' => $this->getChartAcurracy($user), '#timespend' => $this->getChartTimeSpend($user), '#perform' => $this->getChartPerform($user), '#listening' => $this->getChartListening($user), '#reading' => $this->getChartReading($user), '#analytics' => $analytics, '#dataChart' => $this->getChartData($analytics), '#attached' => ['library' => ['iot_user/iot_analytics',],]];
  }
