  ...
  public function ShowSolution (NodeInterface $node)
  {
    ...
    switch ($type) {
      case 'listening':
        ...
        $return = ['#theme' => ['iot_solution_listening'], '#node' => $node, '#result' => $data, '#secs' => $content['secs'], '#audio' => $content['audio'], '#collection_header' => $collection_header, '#answers' => $content['answers'], '#score_table' => $score_table, '#leader_board' => $leader, '#attached' => ['library' => ['iot_quiz/iot_result', 'iot_quiz/iot_frontend',],]];
        break;
      case 'reading':
        $service = \Drupal::service('iot_quiz.quizservice');
        ...
        $score_table = render($score_table_theme);
        $return = ['#theme' => ['iot_solution_reading'], '#node' => $node, '#result' => $data, '#collection_header' => $collection_header, '#secs' => $content['secs'], '#answers' => $content['answers'], '#score_table' => $score_table, '#leader_board' => $service_question->getLeaderBoard($node->id()), '#attached' => ['library' => ['iot_quiz/iot_result',],]];
        break;
    }
    return $return;
  }
  ...
  public function printDownload (NodeInterface $node)
  {
    ...
    switch ($type) {
      case 'listening':
        ...
        $leader = $service_question->getLeaderBoard($node->id());
        $score_table = render($score_table_theme);
        $return = ['#theme' => ['iot_print_listening'], '#node' => $node, '#result' => $data, '#secs' => $content['secs'], '#audio' => $content['audio'], '#collection_header' => $collection_header, '#answers' => $content['answers'], '#score_table' => $score_table, '#leader_board' => $leader, '#attached' => ['library' => ['iot_quiz/iot_printpdf',],]];
        break;
      case 'reading':
        $service = \Drupal::service('iot_quiz.quizservice');
        $content = $service->get_question($node, 'reading', 'solution');
        ...
        $score_table_theme = ['#theme' => 'iot_result_question', '#result' => $data, '#class' => 'green',];
        $score_table = render($score_table_theme);
        $return = ['#theme' => ['iot_solution_reading'], '#node' => $node, '#result' => $data, '#collection_header' => $collection_header, '#secs' => $content['secs'], '#answers' => $content['answers'], '#score_table' => $score_table, '#leader_board' => $service_question->getLeaderBoard($node->id()), '#attached' => ['library' => ['iot_quiz/iot_printpdf',],]];
        break;
    }
    return $return;
  }
