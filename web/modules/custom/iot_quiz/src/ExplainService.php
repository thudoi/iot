...
class ExplainService
{
  ...
  {
    $sids = \Drupal::entityQuery('node')->condition('type', 'question')->condition('field_section', $sid)->execute();
      ...
      foreach ($blocks as $block) {
        ...
        if ($type == 'radio' || $type == 'checkbox') {
          ...
          for ($i = $exp[0]; $i <= $exp[1]; $i++) {
            $paragraph = Paragraph::create(['type' => 'explanation', 'field_explanation' => ["value" => $data[$i], "format" => "full_html"], 'field_number' => ["value" => $i,],]);
            $paragraph->save();
