...
class EntityAliasTypeBase extends ContextAwarePluginBase implements AliasTypeInterface, AliasTypeBatchUpdateInterface, ContainerFactoryPluginInterface {
  ...
  protected function bulkUpdate(array $ids, array $options = []) {
    ...
    $entities = $this->entityTypeManager->getStorage($this->getEntityTypeId())
      ->loadMultiple($ids);
    ...
    if (!empty($options['message'])) {
      drupal_set_message(\Drupal::translation()
        ->formatPlural(count($ids), 'Updated 1 %label URL alias.', 'Updated @count %label URL aliases.'), ['%label' => $this->getLabel()]);
    }
  ...
  protected function bulkDelete(array $pids_by_id) {
    ...
    $states = $this->keyValue->get($collection)
  ...
  public function calculateDependencies() {
    $dependencies = [];
    $dependencies['module'][] = $this->entityTypeManager->getDefinition($this->getEntityTypeId())
      ->getProvider();
    return $dependencies;
  }
