...
class PathautoBulkUpdateForm extends FormBase
{
  ...
  public function buildForm (array $form, FormStateInterface $form_state)
  {
    ...
    $form['update'] = ['#type' => 'checkboxes', '#title' => $this->t('Select the types of paths for which to generate URL aliases'), '#options' => [], '#default_value' => [],];
    ...
    $form['action'] = ['#type' => 'radios', '#title' => $this->t('Select which URL aliases to generate'), '#options' => ['create' => $this->t('Generate a URL alias for un-aliased paths only')], '#default_value' => 'create',];
    ...
    if ($config->get('update_action') == PathautoGeneratorInterface::UPDATE_ACTION_NO_NEW) {
      ...
      $form['warning'] = ['#markup' => $this->t('<a href=":url">Pathauto settings</a> are set to ignore paths which already have a URL alias. You can only create URL aliases for paths having none.', [':url' => Url::fromRoute('pathauto.settings.form')->toString()]),];
    } else {
    ...
    $form['actions']['submit'] = ['#type' => 'submit', '#value' => $this->t('Update'),];
  ...
  {
    $batch = ['title' => $this->t('Bulk updating URL aliases'), 'operations' => [['Drupal\pathauto\Form\PathautoBulkUpdateForm::batchStart', []],], 'finished' => 'Drupal\pathauto\Form\PathautoBulkUpdateForm::batchFinished',];
