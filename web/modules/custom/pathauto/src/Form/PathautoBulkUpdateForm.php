...
class PathautoBulkUpdateForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($config->get('update_action') == PathautoGeneratorInterface::UPDATE_ACTION_NO_NEW) {
      ...
      $form['warning'] = [
        '#markup' => $this->t('<a href=":url">Pathauto settings</a> are set to ignore paths which already have a URL alias. You can only create URL aliases for paths having none.', [
          ':url' => Url::fromRoute('pathauto.settings.form')->toString(),
        ]),
      ];
    }
    else {
