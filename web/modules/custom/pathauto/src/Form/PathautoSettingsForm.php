...
class PathautoSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $max_length = \Drupal::service('pathauto.alias_storage_helper')
      ->getAliasSchemaMaxlength();

    $help_link = '';
    if (\Drupal::moduleHandler()->moduleExists('help')) {
      $help_link = ' ' . $this->t('See <a href=":pathauto-help">Pathauto help</a> for details.', [
          ':pathauto-help' => Url::fromRoute('help.page', ['name' => 'pathauto'])
            ->toString(),
        ]);
    }
    ...
    if (\Drupal::moduleHandler()->moduleExists('redirect')) {
      $description .= ' ' . $this->t('The <a href=":url">Redirect module settings</a> affect whether a redirect is created when an alias is deleted.', [
