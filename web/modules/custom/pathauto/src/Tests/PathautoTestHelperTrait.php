...
trait PathautoTestHelperTrait
{
  ...
  protected function createPattern ($entity_type_id, $pattern, $weight = 10)
  {
    ...
    $pattern = PathautoPattern::create(['id' => Unicode::strtolower($this->randomMachineName()), 'type' => $type, 'pattern' => $pattern, 'weight' => $weight,]);
    $pattern->save();
    return $pattern;
  }
  ...
  protected function addBundleCondition (PathautoPatternInterface $pattern, $entity_type, $bundle)
  {
    ...
    $pattern->addSelectionCondition(['id' => $plugin_id, 'bundles' => [$bundle => $bundle,], 'negate' => FALSE, 'context_mapping' => [$entity_type => $entity_type,]]);
  }
  ...
  public function assertAlias ($source, $expected_alias, $langcode = Language::LANGCODE_NOT_SPECIFIED)
  {
    ...
    $this->assertEqual($expected_alias, \Drupal::service('path.alias_manager')->getAliasByPath($source, $langcode), t("Alias for %source with language '@language' is correct.", ['%source' => $source, '@language' => $langcode]));
  }
  ...
  {
    $values += ['name' => Unicode::strtolower($this->randomMachineName(5)), 'vid' => $vocabulary->id(),];
  ...
  public function assertEntityPattern ($entity_type, $bundle, $langcode = Language::LANGCODE_NOT_SPECIFIED, $expected)
  {
    ...
    $values = ['langcode' => $langcode, \Drupal::entityTypeManager()->getDefinition($entity_type)->getKey('bundle') => $bundle,];
