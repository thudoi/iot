...
class PathautoKernelTest extends KernelTestBase
{
  ...
  public function testPatternLoadByEntity ()
  {
    ...
    $this->addBundleCondition($pattern, 'node', 'article');
    $pattern->addSelectionCondition(['id' => 'language', 'langcodes' => ['en' => 'en',], 'negate' => FALSE, 'context_mapping' => ['language' => 'node:langcode:language',]]);
    ...
    $tests = [['entity' => 'node', 'values' => ['title' => 'Article fr', 'type' => 'article', 'langcode' => 'fr',], 'expected' => '/article/[node:title]',], ['entity' => 'node', 'values' => ['title' => 'Article en', 'type' => 'article', 'langcode' => 'en',], 'expected' => '/article/en/[node:title]',], ['entity' => 'node', 'values' => ['title' => 'Article und', 'type' => 'article', 'langcode' => LanguageInterface::LANGCODE_NOT_SPECIFIED,], 'expected' => '/article/[node:title]',], ['entity' => 'node', 'values' => ['title' => 'Page', 'type' => 'page',], 'expected' => '/[node:title]',], ['entity' => 'user', 'values' => ['name' => 'User',], 'expected' => '/users/[user:name]',],];
    foreach ($tests as $test) {
      ...
      $this->assertIdentical($actual->getPattern(), $test['expected'], t("Correct pattern returned for @entity_type with @values", ['@entity' => $test['entity'], '@values' => print_r($test['values'], TRUE),]));
    }
  }
  ...
  public function testSameTitleDifferentLanguages ()
  {
    ...
    $edit = ['title' => 'Sample page', 'type' => 'page', 'langcode' => 'en',];
  ...
  public function testCleanString ()
  {
    ...
    foreach ($tests as $input => $expected) {
      ...
      $this->assertEqual($output, $expected, t("Drupal::service('pathauto.alias_cleaner')->cleanString('@input') expected '@expected', actual '@output'", ['@input' => $input, '@expected' => $expected, '@output' => $output,]));
    }
  }
  ...
  public function testCleanAlias ()
  {
    ...
    foreach ($tests as $input => $expected) {
      ...
      $this->assertEqual($output, $expected, t("Drupal::service('pathauto.generator')->cleanAlias('@input') expected '@expected', actual '@output'", ['@input' => $input, '@expected' => $expected, '@output' => $output,]));
    }
  }
  ...
  public function testNoTokensNoAlias ()
  {
    ...
    $this->nodePattern->setPattern('/content/[node:body]')->save();
  ...
  {
    $this->config('pathauto.settings')->set('max_length', 26)->save();
  ...
  public function testSettingChangeInvalidatesCache ()
  {
    ...
    $this->config('pathauto.settings')->set('enabled_entity_types', ['user', 'entity_test'])->save();
  ...
  protected function drupalCreateNode (array $settings = [])
  {
    ...
    $settings += ['title' => $this->randomMachineName(8), 'type' => 'page',];
