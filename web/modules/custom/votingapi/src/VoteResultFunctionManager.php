...
class VoteResultFunctionManager extends DefaultPluginManager
{
  ...
  public function getResults ($entity_type_id, $entity_id)
  {
    ...
    $result = db_select('votingapi_result', 'v')->fields('v', ['type', 'function', 'value'])->condition('entity_type', $entity_type_id)->condition('entity_id', $entity_id)->execute();
  ...
  {
    db_delete('votingapi_result')->condition('entity_type', $entity_type_id)->condition('entity_id', $entity_id)->condition('type', $vote_type)->execute();

    $vote_ids = \Drupal::entityQuery('vote')->condition('entity_type', $entity_type_id)->condition('entity_id', $entity_id)->condition('type', $vote_type)->sort('type')->execute();
  ...
  protected function performAndStore ($votes)
  {
    ...
    foreach ($this->getDefinitions() as $plugin_id => $definition) {
      ...
      db_insert('votingapi_result')->fields(['entity_id' => $entity_id, 'entity_type' => $entity_type_id, 'type' => $vote_type, 'function' => $plugin_id, 'value' => $plugin->calculateResult($votes), 'timestamp' => REQUEST_TIME,])->execute();
    }
  }
}
