...
class VoteStorage extends SqlContentEntityStorage implements VoteStorageInterface
{
  ...
  {
    $query = \Drupal::entityQuery('vote')->condition('user_id', $uid);
    if ($vote_type_id) {
  ...
  function getVotesSinceMoment ()
  {
    ...
    return \Drupal::entityQueryAggregate('vote')->condition('timestamp', $last_cron, '>')->groupBy('entity_type')->groupBy('entity_id')->groupBy('type')->execute();
  }

  function deleteVotesForDeletedEntity ($entity_type_id, $entity_id)
  {
    $votes = \Drupal::entityQuery('vote')->condition('entity_type', $entity_type_id)->condition('entity_id', $entity_id)->execute();
    if (!empty($votes)) {
      entity_delete_multiple('vote', $votes);
    }
    db_delete('votingapi_result')->condition('entity_type', $entity_type_id)->condition('entity_id', $entity_id)->execute();
  }
