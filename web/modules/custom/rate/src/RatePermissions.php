      ...
      foreach ($enabled_types_bundles as $entity_type_id => $bundles) {
        foreach ($bundles as $bundle_id) {
          if ($bundle_id == $entity_type_id) {
            ...
            $permissions[$perm_index] = ['title' => $this->t('Can vote on :type', [':type' => $entity_type_id,]),];
          } else {
            $perm_index = 'cast rate vote on ' . $entity_type_id . ' of ' . $bundle_id;
            $permissions[$perm_index] = ['title' => $this->t('Can vote on :type type of :bundle', [':bundle' => $bundle_id, ':type' => $entity_type_id,]),];
          }
        }
      }
    }
