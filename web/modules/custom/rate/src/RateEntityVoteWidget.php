...
class RateEntityVoteWidget
{
  ...
  public function buildRateVotingWidget ($entity_id, $entity_type_id, $bundle)
  {
    ...
    if (isset($enabled_types_bundles[$entity_type_id]) && in_array($bundle, $enabled_types_bundles[$entity_type_id])) {
      ...
      $vote_ids = $vote_storage->getUserVotes($this->accountProxy->id(), NULL, $entity_type_id, $entity_id);
      ...
      if ($this->config->get('widget_type') == 'fivestar') {
        $output['votingapi_links'] = ['#theme' => $rate_theme, '#star1_votes' => $vote_sums['star1'], '#star2_votes' => $vote_sums['star2'], '#star3_votes' => $vote_sums['star3'], '#star4_votes' => $vote_sums['star4'], '#star5_votes' => $vote_sums['star5'], '#use_ajax' => $use_ajax, '#can_vote' => $user_can_vote, '#has_voted' => $has_voted, '#entity_id' => $entity_id, '#entity_type_id' => $entity_type_id, '#attributes' => ['class' => ['links', 'inline']], '#cache' => ['tags' => ['vote:' . $bundle . ':' . $entity_id]],];
      } else {
        $output['votingapi_links'] = ['#theme' => $rate_theme, '#up_votes' => $vote_sums['up'], '#down_votes' => $vote_sums['down'], '#use_ajax' => $use_ajax, '#can_vote' => $user_can_vote, '#has_voted' => $has_voted, '#entity_id' => $entity_id, '#entity_type_id' => $entity_type_id, '#attributes' => ['class' => ['links', 'inline']], '#cache' => ['tags' => ['vote:' . $bundle . ':' . $entity_id]],];
      }
    }
