...
class RateVote implements ContainerInjectionInterface {
  ...
  public function vote($entity_type_id, $vote_type_id, $entity_id, $show_messages = TRUE) {
    ...
    if (!$is_bot_vote && $this->accountProxy->hasPermission('cast rate vote on ' . $entity_type_id . ' of ' . $entity->bundle())) {
      ...
      $vote_ids = $vote_storage->getUserVotes($this->accountProxy->id(), $vote_type_id, $entity_type_id, $entity_id);
      ...
      if (empty($vote_ids)) {
        ...
        if ($show_messages) {
          drupal_set_message(t('Your :type vote was added.', [':type' => $vote_type_id,]));
        }
      } // Otherwise, inform user of previous vote.
      elseif ($show_messages) {
        drupal_set_message(t('You are not allowed to vote the same way multiple times.'), 'warning');
      }
    }
  }
  ...
  public function undoVote($entity_type_id, $entity_id, $show_messages = TRUE) {
    ...
    if (!$is_bot_vote && $this->accountProxy->hasPermission('cast rate vote on ' . $entity_type_id . ' of ' . $entity->bundle())) {
      $vote_storage = $this->entityTypeManager->getStorage('vote');
      $vote_result = $vote_storage->getUserVotes($this->accountProxy->id(), NULL, $entity_type_id, $entity_id);
