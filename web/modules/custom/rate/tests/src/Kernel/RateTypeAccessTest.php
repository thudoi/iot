...
class RateTypeAccessTest extends KernelTestBase
{
  ...
  protected function setUp ()
  {
    ...
    $this->logged_in_user = $this->drupalCreateUser(['view rate results page',]);
  ...
  public function testRateTypeAccess ()
  {
    ...
    foreach ($rate_types as $rate_type) {
      ...
      $this->assertTrue($this->accessHandler->access($vote_type, 'view', $this->logged_in_user), $this->t('Logged in user can see vote of type @rate_type', ['@rate_type' => $rate_type]));

      // Confirm that the anonymous_user cannot access the vote type info.
      $this->assertFalse($this->accessHandler->access($vote_type, 'view', $this->anonymous_user), $this->t('Anonymous user cannot see vote of type @rate_type', ['@rate_type' => $rate_type]));
    }
    ...
    $this->assertFalse($this->accessHandler->access($fake_vote_type, 'view', $this->logged_in_user), $this->t('Logged in user cannot see vote of type @rate_type', ['@rate_type' => $fake_vote_type->id()]));

    $this->assertFalse($this->accessHandler->access($fake_vote_type, 'view', $this->anonymous_user), $this->t('Anonymous user cannot see vote of type @rate_type', ['@rate_type' => $fake_vote_type->id()]));
  }
